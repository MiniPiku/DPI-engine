...
|---------|---------|----------|
| Simple (Single-threaded) | `python -m dpi.dpi_simple` | Learning, small captures |
| Multi-threaded | `python -m dpi` | Production, large captures |
...
│   ├── __main__.py             # Default entry (multi-threaded)
│   ├── dpi_engine.py           # ★ MULTI-THREADED VERSION ★
│   ├── dpi_simple.py           # ★ SIMPLE VERSION ★
│   ├── pcap_reader.py          # PCAP file reading
│   ├── packet_parser.py        # Network protocol parsing
│   ├── sni_extractor.py        # TLS/HTTP/DNS inspection
│   ├── types.py                # FiveTuple, AppType, classification
│   ├── rules.py                # Blocking rules
│   └── utils.py                # Shared helpers
│
├── generate_test_pcap.py       # Creates test data
├── test_dpi.pcap               # Sample capture (after running generator)
...
# Create sample capture (optional)
...
```bash
python -m dpi input.pcap output.pcap --lbs 4 --fps 4
...
Open **http://127.0.0.1:5000** in your browser. Upload a PCAP, configure blocking rules, and download the filtered capture.
